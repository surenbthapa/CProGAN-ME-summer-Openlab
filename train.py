...
# To run, comment/uncomment appropriate lines in config.py and launch training
...
def train_progressive_gan(
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=dataset.load_dataset(data_dir=config.data_dir, verbose = True, **config.dataset), minibatch_size=minibatch_split, reals=reals_gpu, **config.G_loss)
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=dataset.load_dataset(data_dir=config.data_dir, verbose = True, **config.dataset), minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, **config.D_loss)
    ...
    latents = np.random.randn(120, 1, 128, 128)
